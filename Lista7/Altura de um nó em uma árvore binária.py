...
class Tree():
    ...
    def Search(_self_,N,x=0):
        if x==0:
            x=_self_.Root
        if x==None or N==x.Key:
            return x
        if N<x.Key:
            return _self_.Search(N,x.L)
        else:
            return _self_.Search(N,x.R)

    def Height(_self_,N):
        if N==None:
            return -1
        h1=_self_.Height(N.L)
        h2=_self_.Height(N.R)
        if h1>=h2:
            return(1+h1)
        return(1+h2)
...
N=BT.Search(int(input()))
if N==None:
    print("NAO ESTA NA ARVORE")
    print("-1")
else:
    print("ESTA NA ARVORE")
    R=BT.Height(BT.Root)
    L=BT.Height(N)
    Deep=R-L
    print(Deep)
